...
class ConsultasEmpleadoController extends Controller
{
    ...
    public function inasistenciasEmpleados(){
        ...
        foreach($consulta as $c){
        	switch ($c->fk_zona_empleado_2) {
        		case 1:
        			$c->setAttribute("zo_nombre","deposito A"); break;
        		case 2:
        			$c->setAttribute("zo_nombre","deposito B"); break;
        		case 3:
        			$c->setAttribute("zo_nombre","deposito C"); break;
        		case 4:
        			$c->setAttribute("zo_nombre","zona de carga C1"); break;
        		case 5:
        			$c->setAttribute("zo_nombre","zona de carga C2"); break;
        		case 6:
        			$c->setAttribute("zo_nombre","zona administrativa"); break;
        		case 7:
        			$c->setAttribute("zo_nombre","zona para clientes"); break;
        	}
        	$i++;
        }
    ...
    public function inasistenciasEmpleados2(){
        $consulta = Asistencia::join('zona_empleado_horario','zo_em_ho_clave','=','asistencia.fk_zo_em_ho_5')->join('empleado','empleado.em_clave','=','zona_empleado_horario.fk_zona_empleado_3')->select(DB::raw('count(asistencia.*) as cant, em_nombre, em_cedula, em_nacionalidad'))->where('a_check', '=', null)->groupBy('em_nombre', 'em_nacionalidad', 'em_cedula')->get();
        return view('consulta15')->with(compact('consulta'));
    } 

     //Listado de inasistencia indicando el horario asignado al empleado. Creo que se responde con la primera al menos que quiera un buscador del empleado y que salga las inasistencias de solo ese empleado en especifico
}
